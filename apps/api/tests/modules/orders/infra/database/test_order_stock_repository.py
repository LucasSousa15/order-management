...
def test_create_order_updates_product_stock_in_database() -> None:
    ...
    try:
        ...
        persisted_order = order_repository.get_by_id(order.id)  # type: ignore[arg-type]
        ...
        DeleteOrder(order_repository, product_repository).execute(order.id)  # type: ignore[arg-type]
        session.expire_all()
